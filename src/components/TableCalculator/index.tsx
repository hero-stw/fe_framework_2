...
const TableCalculator = ({}: Props) => {
  ...
  const Lap = () => {
    ...
    if (lap.length <= 4) {
      ...
      setLap([...lap, lapNum]);
      setTime(
        new Date(lapNum).getSeconds() + "s" + new Date(lapNum).getMilliseconds()
      );
  ...
  useEffect(() => {
    if (calculation.length != 0 && notification != true) {
      ref.current.focus();
    }
  }, [calculation.length]);
    ...
    <div>
        ...
        showTotal ? (
          ...
          >
            ...
            <div className="flex justify-center bg-yellow-500 h-10 w-36 rounded-lg">
              <button className="text-black font-bold" onClick={() => close()}>
                Check Answers
              </button>
            </div>
          </div>
      ...
      <div className={show ? "relative" : "relative hidden"}>
        <div
          id="popup-modal"
          className={
            "grid place-center overflow-y-auto overflow-x-hidden fixed right-0 left-0 z-50 md:inset-0 h-modal md:h-full bg-gray-500/40"
          }
        >
          <div className="relative p-4 w-full max-w-md h-full md:h-auto mx-auto">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="">
                <div className="p-6 text-center bg-[#4192EC] mt-56">
                  <svg
                    aria-hidden="true"
