...
const LeaderBoard = (props: Props) => {
  ...
  const showSidebar = () => {
    setSidebar(!sidebar), setIcon(!icon);
  };
  ...
  const sortUsers = (index) => {
    ...
    switch (index) {
      ...
      default:
        return (
          <div className="bg-gray-500 rounded-full h-8 w-8 flex justify-center items-center ml-2 text-white">
            {++index}
          </div>
        );
    }
    ...
    <div className="h-full">
      <div className={style.nav__menu__icon}>
        <button onClick={showSidebar}>
          {icon ? (
            <img
              src="https://cdn-icons-png.flaticon.com/512/6345/6345315.png"
              alt="Rank"
              width="50px"
            />
          ) : (
      ...
      <div className={sidebar ? style.nav__menu__active : style.nav__menu}>
        <div
          className="absolute text-xl cursor-pointer right-1 top-1"
          onClick={showSidebar}
        >
          <AiFillCloseCircle />
        </div>
        <div className={style.result}>
          <div className={style.result__title}>Wall of fame</div>
          ...
          {users.map((item, index) => (
            <div key={item._id} className={style.result__value}>
              <div className="">{sortUsers(index)}</div>
              <div className="-mt-1">
                <div className={style.result__username}>{item.userName}</div>
                <div className={style.result__stat}>
                  <div className={style.result__percent}>
                    {roundTo2(item.error)} %
                  </div>
                  <div className={style.result__time}>{item.duration}</div>
                </div>
              </div>
            </div>
          ))}
